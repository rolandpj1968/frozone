...
# 1. Collect all lines, with file/line metadata. `strip()` makes the
# match indentation-independent — two identical blocks at different
# nesting levels count as the same fingerprint.
lines = []   # list of stripped text
meta  = []   # parallel list of (file, lineno)
for dirpath, dirnames, filenames in os.walk(ROOT):
    for fn in filenames:
        if not fn.endswith(('.rb',)): continue
        path = os.path.join(dirpath, fn)
        with open(path, 'r', errors='replace') as f:
            for i, raw in enumerate(f, 1):
                s = raw.strip()
                # Skip noise: empty, lone block keywords / delimiters,
                # bare comments, and divider lines (---, ===, ###).
                if not s: continue
                if s in ('end', 'else', 'begin', '}', '{'): continue

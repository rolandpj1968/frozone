# Frozone Crystal Compilation — Design Notes

> Pre-implementation design exploration and architectural analysis.
> For the working implementation and current status, see [compilation.md](compilation.md).
...
Items marked as open, deferred, or future throughout the design notes:

- **Single-file vs library codegen:** generate a single-file Crystal program or a Crystal library? The library approach is more compositional but the single-file approach is simpler to start with. (Compilation pipeline section)

- **Block representation:** option 3 (`yield` for most cases, explicit `&block` only for stored procs) is "probably right" but not validated. Fixed-arity overloads and array-args are fallbacks. (Block representation section)

- **Integer boxing performance:** Crystal's escape analysis should handle unboxing in natural-form paths, but this needs benchmarking on real numeric workloads. (Integer boxing section)

- **Vtable explosion mitigation:** module-based vtable splitting (interface traits) is designed but not implemented. The polymorphic-call-only stub strategy needs call-graph analysis. (Vtable size / explosion sections)

- **Singleton methods on arbitrary objects:** inter-procedural analysis for `def obj.method` on non-`self` receivers is non-trivial; the "local variable that never escapes" heuristic is a starting point. (Singleton classes section)

- **`eval` runtime fallback (v2+):** bundling the Frozone interpreter as a library for truly dynamic eval requires FFI bridging in both directions (Crystal <-> interpreter), local variable frame boxing/unboxing, and solving the parser dependency circle via Target A. (eval fallback sections)

- **`eval` static analysis:** cases 1-3 (literal, constprop, small-value-set) are tractable; case 4 (ruby/spec structural patterns) needs pattern-matching; case 5 (truly dynamic) is deferred until Target A. (eval interpolates section)

- **`respond_to_missing?` in compiled code:** the bitset fast-path falls back to `respond_to_missing?` but this is untested. (respond_to? section)

- **`method_missing` codegen:** needs generated `case` dispatch over known method names; design exists but not implemented. (Frozone self-compilation audit)

- **`send` with dynamic method names:** bounded by the closed world; `case` dispatch is sufficient but the set may be large. (Frozone self-compilation audit)

- **Target A (Frozone-as-interpreter compiled to Crystal):** the practical near-term goal; exercises the compiler on a large real codebase. Required before the `eval` fallback can work. (Frozone self-compilation section)

- **Target B (closed-world program compilation):** the longer-term goal for arbitrary programs. (Frozone self-compilation section)

- **Compiled mspec runner:** ~200-300 line minimal spec runner covering `describe`/`it`/matchers; Natalie's approach validates this is feasible. Needs eval-analysis pre-pass for spec files. (Testing with mspec section)

- **`const_get`/`const_set` with dynamic names:** `case` dispatch over all known constants, or prohibited. (Constant lookup section)

...
struct Ruby_GCNode {
  ...
  int64_t gc_populate(int64_t depth, int64_t node) {
    if ((depth > 0LL)) {
      depth = (depth - 1LL); left = Ruby_GCNode(); right = Ruby_GCNode(); node.left=(left); node.right=(right); gc_populate(depth, left); gc_populate(depth, right);
    }
  }
  ...
  int64_t gc_make_tree(int64_t depth) {
    if ((depth <= 0LL)) {
      ...
      Ruby_GCNode(gc_make_tree((depth - 1LL)), gc_make_tree((depth - 1LL)));
    }
  }
...
int main() {
  ...
  Ruby_GCNode long_lived_tree = Ruby_GCNode();

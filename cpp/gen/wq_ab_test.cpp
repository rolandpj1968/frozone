...
int main() {
  int64_t source = /* UNSUPPORTED: GlobalVariableRead */.read();
  int64_t parser = /* UNSUPPORTED: ConstantPath */.new();
  parser.diagnostics().consumer=(RUBY_NIL);
  parser.diagnostics().all_errors_are_fatal=(false);
  int64_t buf = /* UNSUPPORTED: ConstantPath */.new("(stdin)");
  int64_t ast = parser.parse(buf);
  if (ast) {
    printf("%.9f\n", (double)(ast.to_sexp()));
  }
  return 0;

...
extern ParseError_eigenclass ParseError_CLASS;
...
struct ParseError : StandardError {
  using StandardError::StandardError;
  const char* ruby_class_name() const override { return "ParseError"; }
  template<class... __Ts__, std::enable_if_t<(... && std::is_pointer_v<__Ts__>), int> = 0> ParseError(__Ts__...) {}
};
...
struct ParseError_eigenclass : Class {
  using Class::Class;
  const char* ruby_class_name() const override { return "ParseError"; }
  template<class... __Ts__, std::enable_if_t<(... && std::is_pointer_v<__Ts__>), int> = 0> ParseError_eigenclass(__Ts__...) {}
};
...
inline ParseError_eigenclass ParseError_CLASS;
...
struct MainObject : Object {
  ...
  void __top_level__() {
    (ruby_puts((new String("loaded wq parser", 16))), nil_instance());
  }
};

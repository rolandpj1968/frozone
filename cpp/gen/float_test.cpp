...
struct BasicObject {
  ...
  virtual BasicObject* m_mul(BasicObject*) { return method_missing("*"); }
  virtual BasicObject* m_minus(BasicObject*) { return method_missing("-"); }
  virtual BasicObject* m_div(BasicObject*) { return method_missing("/"); }
  virtual BasicObject* m_lt(BasicObject*) { return method_missing("<"); }
  virtual BasicObject* m_neg() { return method_missing("-@"); }
...
struct MainObject : Object {
  ...
  void __top_level__() {
    ...
    ruby_puts(a->m_plus(b));
    ruby_puts(a->m_mul(b));
    ruby_puts(b->m_minus(a));
    ruby_puts(b->m_div(a));
    ruby_puts(a->m_lt(b));
    ruby_puts(a->m_eq_q((new Float(1.5))));
    ruby_puts(a->m_neg());
    BasicObject* h = (new Hash({{(new Float(0.1)), intern("one_tenth")}, {(new Float(0.5)), intern("half")}}));
    ruby_puts(h->m_aref((new Float(0.5))));
    ruby_puts(h->m_aref((new Float(0.1))));
  }
};

...
(begin
    ...
    class Ruby_TypeInference < RubyObject
      ...
      def run(*, iterations : RubyObject = (RubyInteger.new(10_i64)))
        ...
        iterations.times() {         changed = RUBY_FALSE
        ...
        @_expr_cache = RubyHash.new
        @user_classes.each() { |cname, klass|         changed = (changed | propagate_ivars(cname, klass))
      ...
      def seed_constants
        @constants.each() { |name, value|         ty = vm_object_type(value)
        if (ty).truthy?
      ...
      def update_call_sites(node : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
        walk(node) { |n|         unless n.ruby_is_a?(Ruby_Ast::Ruby_MethodCall)
          next
        end
      ...
      def propagate_kw_args(call : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
        kw_args.each() { |kw_name_node, val_node|         kw_sym = if kw_name_node.ruby_is_a?(Ruby_Ast::Ruby_SymbolLiteral)
          kw_name_node.value
        else
          RUBY_NIL
        end
      ...
      def propagate_free_call_args(call : RubyObject, ctx : Ruby_TypeContext)
        changed = RUBY_FALSE
                (_or67 = call.arg_nodes; _or67.truthy? ? _or67 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        if (ty.bottom?).truthy?
          next
        end
      ...
      def propagate_free_call_args(call : RubyObject, ctx : RubyObject)
        changed = RUBY_FALSE
                (_or68 = call.arg_nodes; _or68.truthy? ? _or68 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        if (ty.bottom?).truthy?
          next
        end
        changed = (changed | @env.join!(RubyArray.new([Ruby_Sym_26, call.name, i] of RubyObject), ty))
      ...
      def propagate_constructor_args(call : RubyObject, ctx : Ruby_TypeContext, class_sym : RubyObject)
        ...
        changed = RUBY_FALSE
                (_or70 = call.arg_nodes; _or70.truthy? ? _or70 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
      ...
      def propagate_constructor_args(call : RubyObject, ctx : RubyObject, class_sym : RubyObject)
        ...
        changed = RUBY_FALSE
                (_or72 = call.arg_nodes; _or72.truthy? ? _or72 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
          changed = (changed | @env.join!(RubyArray.new([Ruby_Sym_28, class_sym, i, ctor_ctx] of RubyObject), ty))
        end }
        changed
      ...
      def propagate_class_method_args(call : RubyObject, ctx : Ruby_TypeContext, mkey : RubyObject)
        changed = RUBY_FALSE
                (_or73 = call.arg_nodes; _or73.truthy? ? _or73 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
      ...
      def propagate_class_method_args(call : RubyObject, ctx : RubyObject, mkey : RubyObject)
        changed = RUBY_FALSE
                (_or74 = call.arg_nodes; _or74.truthy? ? _or74 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
          changed = (changed | @env.join!(RubyArray.new([Ruby_Sym_26, mkey, i] of RubyObject), ty))
        end }
        changed
      ...
      def propagate_instance_method_args(call : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
                (_or75 = call.arg_nodes; _or75.truthy? ? _or75 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
          changed = (changed | @env.join!(RubyArray.new([Ruby_Sym_26, mkey, i] of RubyObject), ty))
        end }
        changed
      ...
      def propagate_instance_method_args(call : RubyObject, ctx : RubyObject)
        ...
        mkey = RubyArray.new([recv_ty.class_name, call.name] of RubyObject)
        changed = RUBY_FALSE
                (_or76 = call.arg_nodes; _or76.truthy? ? _or76 : (RubyArray.new([] of RubyObject))).each_with_index() { |arg, i|         ty = infer_expr(arg, ctx)
        unless (ty.bottom?).truthy?
          changed = (changed | @env.join!(RubyArray.new([Ruby_Sym_26, mkey, i] of RubyObject), ty))
        end }
        changed
      ...
      def propagate_method(mkey : RubyObject, method : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
                (_or77 = method.optional_kw_params; _or77.truthy? ? _or77 : (RubyArray.new([] of RubyObject))).each() { |kw_name, default_node|         unless (default_node).truthy?
          next
        end
      ...
      def propagate_method(mkey : RubyObject, method : RubyObject, ctx : RubyObject)
        ...
        changed = RUBY_FALSE
                (_or78 = method.optional_kw_params; _or78.truthy? ? _or78 : (RubyArray.new([] of RubyObject))).each() { |kw_name, default_node|         unless (default_node).truthy?
          next
        end
        ty = infer_expr(default_node, ctx)
        unless (ty.bottom?).truthy?
      ...
      def propagate_locals(body : RubyObject, ctx : Ruby_TypeContext)
        ...
        loop do
          ...
          assignments.each() { |name, rhs_nodes|           ty = rhs_nodes.reduce(Ruby_Type::Ruby_BOTTOM) { |acc, rhs|           join(acc, infer_expr(rhs, ctx)) }
          if (ty.bottom?).truthy?
            next
          end
      ...
      def propagate_masgn_from_calls(body : RubyObject, ctx : Ruby_TypeContext)
        changed = RUBY_FALSE
        walk(body) { |node|         unless node.ruby_is_a?(Ruby_Ast::Ruby_MultipleAssignment)
          next
        end
        ...
        targets.each_with_index() { |t, i|         unless ((_and86 = ((t[0_i64] == Ruby_Sym_30) ? RUBY_TRUE : RUBY_FALSE); _and86.truthy? ? (ret_elems[i]) : _and86)).truthy?
          next
        end
      ...
      def propagate_masgn_from_calls(body : RubyObject, ctx : RubyObject)
        ...
        targets.each_with_index() { |t, i|         unless ((_and90 = ((t[0_i64] == Ruby_Sym_30) ? RUBY_TRUE : RUBY_FALSE); _and90.truthy? ? (ret_elems[i]) : _and90)).truthy?
          next
        end
      ...
      def propagate_array_locals(body : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
        assignments.each() { |name, rhs_nodes|         if (param_names.include?(name)).truthy?
          next
        end
        ...
        elem_writes.each() { |key, value_nodes|         if (key.is_a?(RubySymbol) ? RUBY_TRUE : RUBY_FALSE)
      ...
      def propagate_for_targets(body : RubyObject, ctx : Ruby_TypeContext)
        ...
        changed = RUBY_FALSE
        walk(body) { |node|         unless node.ruby_is_a?(Ruby_Ast::Ruby_ForLoop)
          next
        end
        target = node.target
...
RUBY_NIL # UNSUPPORTED: DefinedExprbegin
          ...
          if (["RUBY_NIL # UNSUPPORTED: DefinedExpr"]).truthy?
      ...
      def seed_constructor_params(class_name : RubyObject, param_types : RubyObject)
        changed = RUBY_FALSE
        param_types.each_with_index() { |ty, i|         unless (ty.bottom?).truthy?
      ...
      def propagate_ivars_from_initialize(class_name : RubyObject, init : RubyObject)
        ...
        changed = RUBY_FALSE
        collect_ivar_assignments(init.body).each() { |ivar_name, rhs_nodes|         ty = rhs_nodes.reduce(Ruby_Type::Ruby_BOTTOM) { |acc, rhs|         join(acc, infer_expr(rhs, ctx)) }
        if (ty.bottom?).truthy?
          next
        end
      ...
      def propagate_ivars_from_other_methods(class_name : RubyObject, klass : RubyObject)
        changed = RUBY_FALSE
                (_or115 = klass.methods_table; _or115.truthy? ? _or115 : (RubyHash.new)).each() { |mname, method|         if ((_or116 = (_or117 = ((mname == Ruby_Sym_41) ? RUBY_TRUE : RUBY_FALSE); _or117.truthy? ? _or117 : (((method.ruby_is_a?(Ruby_Vm::Ruby_Method)) ? RUBY_FALSE : RUBY_TRUE))); _or116.truthy? ? _or116 : ((((method.body).truthy?) ? RUBY_FALSE : RUBY_TRUE)))).truthy?
          next
        end
        method_ctx : Ruby_TypeContext = Ruby_TypeContext.new(RubyTuple2.new(class_name, mname), class_name)
        collect_ivar_assignments(method.body).each() { |ivar_name, rhs_nodes|         rhs_nodes.each() { |rhs|         ty = infer_expr(rhs, method_ctx)
      ...
      def propagate_ivars_from_setter_calls(class_name : RubyObject, klass : RubyObject)
        accessor_names : Ruby_Set = Ruby_Set.new
                (_or119 = klass.methods_table; _or119.truthy? ? _or119 : (RubyHash.new)).each_key() { |mn|         if ((_and120 = mn.ruby_to_s.end_with?(Ruby_Str_27); _and120.truthy? ? (((mn != Ruby_Sym_41) ? RUBY_TRUE : RUBY_FALSE)) : _and120)).truthy?
        ...
        changed = RUBY_FALSE
        each_user_method() { |mkey, method|         unless (method.body).truthy?
          next
        end
      ...
      def propagate_ivar_array_elem_writes(class_name : RubyObject, klass : RubyObject)
        changed = RUBY_FALSE
                (_or121 = klass.methods_table; _or121.truthy? ? _or121 : (RubyHash.new)).each() { |mname, method|         unless ((_and122 = method.ruby_is_a?(Ruby_Vm::Ruby_Method); _and122.truthy? ? (method.body) : _and122)).truthy?
          next
        end
        method_ctx = Ruby_TypeContext.new(RubyTuple2.new(class_name, mname), class_name)
        collect_ivar_array_elem_writes(method.body, class_name, method_ctx) { |ivar_name, elem_ty|         current = @env.type_of(RubyArray.new([Ruby_Sym_42, class_name, ivar_name] of RubyObject))
      ...
      def each_user_method(&block)
        ...
        @user_classes.each() { |cname, ck|                 (_or124 = ck.methods_table; _or124.truthy? ? _or124 : (RubyHash.new)).each() { |mn, m|         if m.ruby_is_a?(Ruby_Vm::Ruby_Method)

...
def item_check(left : RubyObject, right : RubyObject)
  if (left.ruby_nil?).truthy?
    return RubyInteger.new(1_i64)
  end
  RubyInteger.new((1_i64 + item_check(left[0_i64], left[1_i64]).to_i64) + item_check(right[0_i64], right[1_i64]).to_i64)
end

def bottom_up_tree(depth : Int64)
  unless (depth > 0_i64)
    return RubyTuple2.new(RUBY_NIL, RUBY_NIL)
  end
  depth = (depth - 1_i64)
  RubyTuple2.new(bottom_up_tree(depth), bottom_up_tree(depth))
end

def bottom_up_tree(depth : RubyObject)
  unless (((depth > RubyInteger.new(0_i64)) ? RUBY_TRUE : RUBY_FALSE)).truthy?
    return RubyTuple2.new(RUBY_NIL, RUBY_NIL)
  end
  depth = RUBY_NIL; depth = (depth - RubyInteger.new(1_i64))
  RubyTuple2.new(bottom_up_tree(depth.to_i64), bottom_up_tree(depth.to_i64))
end
...
Ruby_MAX_DEPTH = RubyInteger.new(14_i64)
Ruby_MIN_DEPTH = RubyInteger.new(4_i64)
Ruby_STRETCH_DEPTH = RubyInteger.new(15_i64)

total = 0_i64
60_i64.times { stretch_tree = bottom_up_tree(Ruby_STRETCH_DEPTH.to_i64)
stretch_tree = RUBY_NIL
long_lived_tree = bottom_up_tree(Ruby_MAX_DEPTH.to_i64)
depth = Ruby_MIN_DEPTH.to_i64
while (depth <= Ruby_MAX_DEPTH.to_i64)
  iterations = (2_i64 ** ((Ruby_MAX_DEPTH.to_i64 - depth) + Ruby_MIN_DEPTH.to_i64))
  check = 0_i64
  (1_i64..iterations).each do |i|
    temp_tree = bottom_up_tree(depth)
    check = (check + item_check(temp_tree[0_i64], temp_tree[1_i64]).to_i64)
  end
  total = (total + check)
  depth = (depth + 2_i64)
end }
ruby_puts(RubyInteger.new(total)); RUBY_NIL

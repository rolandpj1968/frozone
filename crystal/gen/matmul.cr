...
def matgen(n : Int64)
    tmp = RUBY_NIL
  tmp = ((1.0_f64 / n.to_f64) / n.to_f64)
  RubyArray.new(n) { |i|
    RubyArray.new(n) { |j|
      RubyFloat.new(((tmp * (i - j).to_f64) * (i + j).to_f64))
    }
  }
end

def matgen(n : RubyObject)
    tmp = RUBY_NIL
  tmp = ((RubyFloat.new(1.0_f64) / n) / n).to_f64
  RubyArray.new(n) { |i|     RubyArray.new(n) { |j|       ((RubyFloat.new(tmp) *       (i - j)) *       (i + j)) } }
end

def matmul(a : Array(Array(Float64)), b : Array(Array(Float64)))
    m = RUBY_NIL
    n = RUBY_NIL
    loc_p = RUBY_NIL
    c = RUBY_NIL
    ...
    j = RUBY_NIL
  m = a.length.to_i64
  n = a[0_i64].length.to_i64
  loc_p = b[0_i64].length.to_i64
  c = Array(Array(Float64)).new(m) { Array(Float64).new(loc_p, 0.0_f64) }
  (0_i64...m).each do |i|
    ci = c[i]
    ai = a[i]
    k = 0_i64
...
def matmul(a : RubyObject, b : RubyObject)
    m = RUBY_NIL
    n = RUBY_NIL
    loc_p = RUBY_NIL
    c = RUBY_NIL
  ...
  (0_i64...m).each do |i|
    ...
    k = 0_i64
    while (k < n)
      aik = ai[k].as(RubyFloat).to_f64
      bk = b[k].as(RubyArray)
      j = 0_i64
      while (j < loc_p)
        ci[j] += (aik * bk[j].as(RubyFloat).to_f64)
        j = (j + 1_i64)
      end
      k = (k + 1_i64)
    end
...
Ruby_N = RubyInteger.new(200_i64)

last = 0.0_f64
20_i64.times { a = matgen(Ruby_N.to_i64)
b = matgen(Ruby_N.to_i64)
c = matmul(a, b)
last = c[(Ruby_N.to_i64 // 2_i64)][(Ruby_N.to_i64 // 2_i64)].to_f64 }
ruby_puts(RubyFloat.new(last)); RUBY_NIL

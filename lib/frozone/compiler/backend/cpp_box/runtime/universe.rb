        ...
        module Runtime
          ...
          HASH = RubyClass.new(
            ...
            members: [
              ...
              "    // Authentic Ruby Hash semantics: hash by the key's own #hash",
              "    // (m_hash returns an Integer; m_hash_value spreads it to size_t),",
              "    // not by C++ object identity. Required for value keys and for",
              "    // KeyWrapper (which defines Ruby hash/eql?, not m_hash_value/==).",
              "    return v->m_hash()->m_hash_value();",
              "  }",
              "};",
              "struct KeyEq {",
              "  bool* by_identity;",
              ...
              "    // Ruby Hash key equality uses eql?, not == (keeps 1 and 1.0 distinct).",
              "    return a->mm_eql_q(&tmp) == true_instance();",
              "  }",
              "};",

class File < IO
  ...
  class << self
    ...
    def binread(path, length = nil, offset = nil) = read(_coerce_path(path), length, offset, mode: 'rb')
    def binwrite(path, content, offset = nil) = IO.write(_coerce_path(path), content, offset, mode: 'wb')
